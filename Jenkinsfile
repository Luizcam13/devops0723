    ...
    stages {
        ...
        stage('Build Images') {
            steps {
                script {
                    // Nome das imagens
                    def imageNameWeb = 'luizhenriquecleto/web'
                    def imageNameDB = 'luizhenriquecleto/db'
                    def imageNameNginx = 'luizhenriquecleto/nginx'

                    // Executa o build das imagens com os respectivos Dockerfiles
                    docker.withRegistry('https://registry.hub.docker.com', 'dockerhubluizhenrique') {
